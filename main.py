...
# CONFIGURATION
# ---------------------
BOT_TOKEN = os.getenv("BOT_TOKEN")  # Only set in Render / Flux env
LIKE_API_URL = "https://rs-like-bot-bd.vercel.app"
API_KEY = "jenil"  
...
def animate_loading(chat_id,message_id,base_text,stages,delay=0.5):
    ...
    try:
        ...
        logging.exception("Loading animation error: %s",e)
...
def start_cmd(message):
    bot.reply_to(message,"👋 <b>Welcome to RS Like Bot</b>\nSend likes: <code>/like &lt;region&gt; &lt;uid&gt;</code>\nExample: <code>/like bd 123456789</code>",parse_mode="HTML")

@bot.message_handler(commands=['help'])
def help_cmd(message):
    bot.reply_to(message,"📖 <b>Help</b>\n<code>/like &lt;region&gt; &lt;uid&gt;</code> — send likes.",parse_mode="HTML")
...
def like_cmd(message):
    ...
    # ---------------------
    try:
        resp = requests.get(f"{LIKE_API_URL}?server_name={region}&uid={uid}&key={API_KEY}", timeout=25)
    ...
    if status==2:
        ...
        txt=f"╔══════⚠️💖⚠️══════╗\n     ALREADY LIKED\n╚══════⚠️💖⚠️══════╝\n\n" \
            f"🏆 PLAYER DETAILS\n╭──────────────────╮\n  🪪 Name: {nick}\n  🆔 UID: {uid_resp}\n  🌍 Region: {region.upper()}\n╰──────────────────╯\n\n" \
            f"💎 LIKE STATUS\n╭───────────╮\n  💖 Current Likes: {data.get('LikesafterCommand','?')}\n  ⚡ VIP Credit restored\n╰───────────╯\n\n" \
    ...
    fail_txt=f"╔══════❌⚡❌══════╗\n       LIKE FAILED\n╚══════❌⚡❌══════╝\n\n" \
             ...
             f"⚠️ Reason: Invalid UID / Server error\n🔥 Join @CARTOONFUNNY03"
...
def run_bot():
    while True:
        try:
            ...
            logging.exception("Polling crashed, restarting in 5s: %s",e)
            time.sleep(5)

if __name__=="__main__":
    Thread(target=run_web,daemon=True).start()
    logging.info("Flask ping server started")
    run_bot()
